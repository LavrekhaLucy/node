...
class EmailService {
    ...
    public async sendMail<T extends EmailTypeEnum>(type: T, to: string, context: EmailTypeToPayload[T],
    ): Promise<void> {
    ...
    public async sendVerifyEmail<T extends EmailTypeEnum>(type:T, to:string, context:EmailTypeToPayload): Promise<void> {
        const { subject, template } = emailConstants[type];
        context['frontUrl'] = configs.APP_FRONT_URL;
        const options = { to, subject, template, context };
    ...
    //         context: {
    //             name: user.name,
    //             verifyLink: `http://your-site.com/verify-email?token=${token}`,
    //             year: new Date().getFullYear(), // ← ось тут
    //         },
    //     });
    // }
